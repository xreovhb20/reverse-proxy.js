...
var Application={
  ...
  run: function() {
    ...
    var format={
      ...
      asIntegerIfNumeric: function(value) {
    ...
    this._loadConfig()
      ...
      .then(function() {
        // Drop privileges.
        if(program.user) {
          if(!('setuid' in process)) self._log('Changing the process user is not supported on this platform.');
          else {
  ...
  _loadConfig: function() {
    ...
    return stat(fullPath)
      .then(function(stats) {
        ...
        throw new Error(util.format('Configuration file not found: "%s"', fullPath));
      })
      .then(function(paths) {
        ...
        return Promise.all(paths.map(function(file) {
          return new Promise(function(resolve, reject) {
            ...
            if(!('routes' in options) && !('target' in options))
              reject(new Error(util.format('You must provide at least a target or a route table in "%s"', file)));
            else {
              if('ssl' in options) {
                if(('ca' in options.ssl) && Array.isArray(options.ssl.ca))
                  options.ssl.ca=options.ssl.ca.map(function(item) { return fs.readFileSync(item); });
  ...
  _log: function(message) {
    if(!program.silent)
      console.log('[%s] %s', new Date().toUTCString(), typeof message=='function' ? message() : message);
  },
