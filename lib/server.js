...
Server.prototype._getHostname=function(req) {
  if('*' in this._options.routes) return '*';
...
Server.prototype._handleHTTPRequest=function(req, res) {
  ...
  if(hostname in this._options.routes) this._proxyService.web(req, res, { target: this._options.routes[hostname] });
...
Server.prototype._handleWebSocketRequest=function(req, socket, head) {
  var hostname=this._getHostname(req);
  if(hostname in this._options.routes) this._proxyService.ws(req, socket, head, { target: this._options.routes[hostname] });
};
