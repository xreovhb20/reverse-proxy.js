...
class Application {
  ...
  async init(args = {}) {
    ...
    if (typeof args.config == 'string') {
      console.log('init.configFile.readFile');
  ...
  async run() {
    ...
    if (cluster.isMaster) {
      ...
      process.on('SIGINT', () => Object.values(cluster.workers).forEach(worker => {
        worker.kill();
        console.log(`Reverse proxy #${worker.id} killed`);
      }));
    }
    else {
  ...
  async _startServers() {
    ...
    return Promise.all(this.servers.map(server => {
      server
        .on('close', () => console.log(`Reverse proxy #${cluster.worker.id} on ${server.address}:${server.port} closed`))
        .on('error', err => console.error(`#${cluster.worker.id}: ${this.debug ? err : err.message}`))
        .on('listening', () => console.log(`Reverse proxy #${cluster.worker.id} listening on ${server.address}:${server.port}`));
