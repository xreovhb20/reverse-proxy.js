...
class Application {
  ...
  async init(args = {}) {
    if (typeof args.config == 'string') {
      ...
      this.servers = await Application._parseConfiguration(await loadConfig(args.config, 'utf8'));
    }
  ...
  async _parseConfiguration(configuration) {
    ...
    for (let options of servers) {
      ...
      if (typeof options.ssl == 'object' && options.ssl) {
        ...
        for (let key of keys) {
          // if startsWith('base64:') decode else encode
          let cert = await loadCert(options.ssl[key]);
          options.ssl[key] = cert; // TODO ().toString('base64');
        }
      }
    }
